...
# 下面三種方法等價，其中最後一個叫做 set comprehension
...
`dict.fromekeys` 會將第一個位置的參數（必須是一個 [[Iterable vs. Iterator|iterable]]）中的元素按照順序逐一放入一個原本為空的 dictionary，由於在 ==Python 3.7 後，dict 中的 keys 是有順序的==，這個順序會與「每個 key 第一次被塞入 dict 的順序」相同，因此這個做法可以在維持元素在原 iterable 中的順序的情況下，同時保證元素的唯一性。
