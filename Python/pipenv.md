...
建議將 pipenv 全域安裝，這樣無論在什麼 directory 都可以使用 pipenv，下面示範如何使用 pip 安裝：
...
3. 在電腦中的某個 directory 底下產生對應的虛擬環境資料夾

    所有虛擬環境資料夾都會被集中在一個名為 `.virtualenvs` 或 `vertualenvs` 的 directory 底下。
...
然而，如果本機有安裝 [[pyenv]]，則在建立虛擬環境的時候，若目前 Global 環境中沒有指定 `<version>` 的 Python，CLI 會問你要不要先使用 pyenv 全域安裝。
...
若要使用 terminal 進入某個 pipenv 虛擬環境，則須找到 virtualenv folder 中指定之虛擬環境 folder 中的 `/bin/activate` 這個檔案，因此若  terminal 已位在專案跟目錄（即 `Pipfile` 所在之目錄），那就可以結合前面介紹的「查詢虛擬環境資料夾路徑」的指令來進入這個專案專用的虛擬環境：
...
現在使用 `python --version` 查到的就會虛擬環境中 Python 的版本了！這代表整個 terminal 現在的 context 就是在虛擬環境中。
...
1. 找到本專案所屬的 virtualenv folder
2. 檢查有沒有 `Pipfile` 有聲明但不存在於 virtualenv 的套件，若有則將它們先下載並安裝
3. 安裝本次指定的套件至該 folder
...
`Pipfile` 只負責控制「明確指定要用到的套件」的版本，但其內所描述的套件版本可以是模糊的，比如若執行 `pipenv install <package-name>` 時沒有聲明要安裝的套件版本，該套件在 `Pipfile` 中就會以 `<package-name> = "*"` 呈現。
...
此時前面提到的某個 directory 底下的 `virtualenvs` 底下的對應的虛擬環境資料夾就會被刪除，但 `Pipfile` 以及 `Pipfile.lock` 都會留著（必須留著）。

當然你也可以手動找到電腦中 `virtualenvs` 底下的對應的虛擬環境資料夾，然後手動刪除它，比較麻煩而已。
