...
Context manager object 是一種必須擁有 `__enter__` 和 `__exit__` 兩個 methods 的 object，這兩個 methods 扮演著入口和出口的角色。搭配 `with` statement 使用的效果就是，一進入 `with` block，首先 Context manager object 的 `__enter__` method 就會被觸發，離開 `with` block 前的最後一件事，則是觸發 `__exit__` method。

所以如果有什麼事情是在執行某段程式碼前一定要做的，或者有什麼事情是在執行完某段程式碼後一定要做的，就可以使用 context manager object 將那些一定要做的事抽離出來。

某種程度上使用 context manager 可以避免程式撰寫者忘記做一些瑣碎卻必要的事，比如關閉檔案、將某些臨時更動的系統全域變數改回預設值等。
...
在沒有 `with` statement 的時代，初學者可能會這樣讀寫檔案：
...
如果在 `with` block 中的程式丟出 Exception，在離開 `with` block 前還是會 call 一下 Context manager object 的  `__exit__` method。

也就是說，其實 `with` statement 有一部份是由 `try...except...finally..` 構成的。
...
首先，一個 Context Manager Object 的 template 會長得像這樣：
...
在 `with EXPRESSION as TARGET:...` 的語法中，`TARGET` 就是 `__enter__` method 的 return value。比如我現在想要一個 Dog 可以在進入 `with` block 時誕生並在離開 `with` block 前睡著的功能：
...
- `try` block 裡是使用 `yield` 語法回傳 `with EXPRESSION as TARGET:...` 的語法中的 `TARGET`。這裡不能用 `return`（關於 `yield` 的用法詳見 [[Generator and the yield Statement]]）簡言之是因為相較於 `return` 會直接跳出並結束 function，`yield` 回傳值時則是讓 function「暫停」，這樣在跳出 `with` block 時才會回到上次暫停的地方繼續運行完（ 剩下的 `finally` block）。
