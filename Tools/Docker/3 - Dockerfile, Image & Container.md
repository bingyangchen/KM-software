...
將 host filesystem 中的檔案複製到 image 內的 filesystem 中（連同檔案的 metadata，如 permission）。也可以用來複製不同 build stages 間的 image（詳見 [#Multi-Stage Builds](</./Tools/Docker/3 - Dockerfile, Image & Container.md#Multi-Stage Builds>)）。
