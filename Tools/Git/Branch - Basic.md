...
- 多個 branches 可以 [[Branch - merge|merge]]（合併）成一個 branch
...
如果可以將 production code 與 development code 區分成兩個 branches（假設叫 `main` 與 `dev`），新功能開發完畢後先 push 上 `dev`，並針對 `dev` 做好測試，確定沒問題後再將 `dev` [[Branch - merge|merge]] 進 `main`，這樣可以大幅增加 `main` 的穩定性。
