...
一個 Linux OS 中可以存在若干個 users，其中必定會有一個 user 叫做 `root`，`root` 擁有系統中的所有（最高）[[7 - 與 Permission 相關的指令|權限]]，又叫做 superuser。
...
輸入指令後會被要求輸入 `root` user 的密碼，輸入正確後會進入一個新的 shell（屬於當前 shell 的 [[Operating System/Shell/1 - Introduction#Sub-Shell|sub-shell]]）。
