...
`tr` 指令須有 stdin，可以透過 [[L3 - Operators#Piping - ` `|piping]] 或 [[L3 - Operators#Input Redirection - `<`|input redirection]] 兩種方式提供：
...
可以用 `>` 或 `>>` 對 `tr` 指令的 stdout 進行 [[L3 - Operators#Output Redirection - `>`|output redirection]]，比如：
