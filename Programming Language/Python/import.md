...
flowchart TD
    ...
    id2{abc 是否已被 import?}
    id3([不再 import])
    id4{abc 是否存在於<br/>Python 標準函式庫中?}
    id5{查詢 sys.path 中<br/>的所有 directories<br/>從離目前 directory<br/>最近的開始找看看<br/>abc 是否為其中一個<br/>package 或 module?}
    id6([執行 import])
...
在 `a.py` 中 `b.say_hi()` 這段程式碼放入 `if __name__ == "__main__":` ([[Programming Language/Python/零碎筆記.draft#__name__ == "__main__"|詳見此文]]) block 中：
